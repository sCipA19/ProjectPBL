...
CREATE TABLE tb_kelolatatib (
    ...
    tingkat VARCHAR(10),
    kompensasi VARCHAR(250),
    tenggat DATE NOT NULL,  -- Kolom untuk tenggat waktu yang tidak boleh NULL
    pengumpulan VARBINARY(MAX), -- Kolom untuk menyimpan file (biner)
    nama_file VARCHAR(255),    -- Nama file yang di-upload
    tipe_file VARCHAR(50)     -- Tipe file (misalnya jpg, pdf)
);
...
(1, 3, '2245760250', 'Bagas Putra Sofyan', '16', 'Melakukan pelecehan dan/atau tindakan asusila dalam segala bentuk di dalam dan di luar kampus', 'di lihat langsung', 'H. Jainal, S.Kom., M.Kom.'),
(2, 3, '2445760090', 'Daffa Athallah Erlangga', '12', 'Mengotori atau mencoret-coret meja, kursi, tembok, dan lain-lain di lingkungan Polinema', 'terlihat cctv', 'H. Jainal, S.Kom., M.Kom.');

CREATE TABLE tb_laporan_pelanggaran_dosen (
    id_laporan INT IDENTITY(1,1) PRIMARY KEY,     -- Auto-increment ID for the report
    nim NVARCHAR(20) NOT NULL,                    -- Student Identification Number
    ...
    status_id INT NOT NULL DEFAULT 1,             -- Default status is "Proses"
    CONSTRAINT FK_StatusLaporan FOREIGN KEY (status_id) REFERENCES tb_status_laporan(status_id) -- Foreign Key Constraint
);


drop table tb_laporan_pelanggaran_dosen;
drop table tb_status_laporan;

select *from tb_laporan_pelanggaran_dosen;

CREATE TABLE tb_status_laporan (
    status_id INT IDENTITY(1,1) PRIMARY KEY, -- Auto-increment ID for status
    status_name NVARCHAR(50) NOT NULL       -- Status name (e.g., "Proses", "Selesai")
);
